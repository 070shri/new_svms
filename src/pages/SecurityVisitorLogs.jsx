...
import { Search, Download, Calendar as CalendarIcon, User, X, MoreVertical } from "lucide-react";
...
const SecurityVisitorLogs = () => {
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    const matchesSearch = log.fullName?.toLowerCase().includes(searchLower) || log.company?.toLowerCase().includes(searchLower) || log.host?.toLowerCase().includes(searchLower);
    const displayStatus = log.status === "Pre-Registered" ? "Approved" : log.status === "Pending Approval" ? "Pending" : log.status;
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 ml-64 p-8">
        <Header title="Security Logs" subtitle="Visitor entry/exit history" action={<Button icon={Download}>Export</Button>} />
        
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-6 mt-6">
          <div className="flex flex-wrap gap-4 items-center">
            <div className="relative flex-1 min-w-[300px]">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
              <input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none" />
            </div>
          ...
          <div className="flex gap-2 flex-wrap border-t border-gray-50 pt-3 mt-4">
            {filters.map((f) => (
              <button key={f} onClick={() => setActiveFilter(f)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeFilter === f ? "bg-green-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>{f}</button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase">
              <tr><th className="px-6 py-3">Visitor</th><th className="px-6 py-3">Host</th><th className="px-6 py-3">Status</th><th className="px-6 py-3 text-center">Actions</th></tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredLogs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedVisitor(log)}>
                  <td className="px-6 py-4 flex items-center gap-3">
                    {log.photo ? <img src={log.photo} className="w-10 h-10 rounded-full object-cover" /> : <div className="w-10 h-10 bg-green-50 rounded-full flex items-center justify-center text-green-600 font-bold">{log.fullName?.charAt(0)}</div>}
                    <div><p className="font-medium text-gray-900">{log.fullName}</p><p className="text-xs text-gray-500">{log.company}</p></div>
                  </td>
                  <td className="px-6 py-4 text-gray-600">{log.host}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${log.status === "Pre-Registered" ? "bg-blue-100 text-blue-700" : "bg-yellow-100 text-yellow-700"}`}>
                      {log.status === "Pre-Registered" ? "Approved" : log.status === "Pending Approval" ? "Pending" : log.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center"><MoreVertical className="w-4 h-4 mx-auto text-gray-400" /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {selectedVisitor && <VisitorInfoModal visitor={selectedVisitor} onClose={() => setSelectedVisitor(null)} themeColor="bg-green-600" />}
      </div>
    </div>
  );
};
...
export default SecurityVisitorLogs; // (or export VisitorLogs depending on the file)